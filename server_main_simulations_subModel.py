...
#os.chdir('/Users/fan/Documents/Research/HIV-transmission-PoissonProcess/')
...
class LatentPoissonDPHGMM:
    def __init__(self, Priors, K=3, Kmax = 10, linkThreshold=0.6):
        '''
        Initialize an instance of the LatentPoisson Hierarchical GMM model;
    ...
    def fit(self, E, L, D, samples = 1000, burn = 0, thin = 1, random_seed = 42, 
        ...
        if fixed_alloc:
            if len(thresholds) == 2:
                ...
                thresL = thresholds[0]
                thresD = thresholds[1:3]
                self.L, inds, self.muL, self.gammaL = initializeLinkedScore(self.L, thresL)
                self.D, self.indsMF, self.indsFM, self.muD, self.muNegD, self.gammaD = initializeDirectScore2(self.D, inds, Dthres = thresD)
        else:
...
#05/19/2023: simulation for N= 400 as well (was missing from previous cell)
...
# simulate from two different scenarios, version 1 and version 2
...
# 05/08/2023: LOCAL RUN
# add a sub-model baseline method 
# with fixed threshold pre-classification

model = LatentPoissonDPHGMM(Priors = Pr, K=3, Kmax = 10)

# try fixing thresholds
#model.fit(E, L, D, samples=3000, burn=0, random_seed = seed, debugHack=False, fixed_alloc=True)

# try fixing thresholds and tri-split on D scores
model.fit(E, L, D, samples=3000, burn=1000, random_seed = seed, debugHack=False, 
          fixed_alloc=True, thresholds=[0.6, 1/3, 2/3])
...
# 05/19/2023: file name for N=400 case for backward compatibility (for post-processing)
if N == 400:
    flabel = str(array_id)
else:
